...
export default function AdminFinancials() {
  ...
  const fetchFinancials = async () => {
    try {
      ...
      const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/financial/summary`, {
        params: dates,
        headers: { Authorization: `Bearer ${token}` }
      });
      setSummary(response.data.data);
    } catch (error) {
      toast.error("Analytics engine failure");
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="glass-card p-8 md:p-10 rounded-[32px] md:rounded-[48px] bg-white border border-slate-100 shadow-xl relative overflow-hidden group">
              ...
              <div className="flex flex-col relative z-10">
                <span className="text-3xl md:text-4xl font-black text-slate-900 tracking-tighter">Rs.{summary.totalRevenue.toLocaleString()}</span>
                <span className="mt-3 text-[9px] font-black text-slate-400 uppercase tracking-[0.3em] bg-slate-50 py-1.5 px-3.5 rounded-full w-fit">{summary.orderCount} Transactions</span>
              </div>
            </div>
            ...
            <div className="glass-card p-8 md:p-10 rounded-[32px] md:rounded-[48px] bg-white border border-slate-100 shadow-xl relative overflow-hidden group">
              ...
              <div className="flex flex-col relative z-10">
                <span className="text-3xl md:text-4xl font-black text-slate-900 tracking-tighter">Rs.{summary.totalExpenses.toLocaleString()}</span>
            ...
            <div className="glass-card p-8 md:p-10 rounded-[32px] md:rounded-[48px] bg-slate-900 text-white border-none shadow-2xl relative overflow-hidden group">
              ...
              <div className="flex flex-col relative z-10">
                <span className="text-3xl md:text-4xl font-black tracking-tighter text-white">Rs.{summary.totalProfit.toLocaleString()}</span>
                <span className={`mt-3 text-[10px] font-black uppercase tracking-[0.3em] flex items-center gap-2 ${summary.totalProfit >= 0 ? "text-emerald-400" : "text-rose-400"}`}>
                  <div className={`w-1.5 h-1.5 rounded-full ${summary.totalProfit >= 0 ? "bg-emerald-400" : "bg-rose-400"} shadow-[0_0_8px]`} />
                  {summary.totalProfit >= 0 ? "Surplus Threshold Met" : "Capital Variance"}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 md:gap-10">
            <div className="glass-card p-8 md:p-10 rounded-[32px] md:rounded-[48px] bg-white border border-slate-100 shadow-xl relative group overflow-hidden">
              ...
              <div className="space-y-6 relative z-10">
                ...
                <button className="w-full py-4.5 md:py-5 bg-slate-900 text-white rounded-xl md:rounded-2xl font-black text-[10px] uppercase tracking-[0.4em] shadow-xl hover:bg-primary transition-all duration-500 flex items-center justify-center gap-2.5">
                  Export P&L Data
