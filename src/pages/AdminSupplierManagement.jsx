...
export default function AdminSupplierManagement() {
  ...
  const fetchSuppliers = async () => {
    try {
      ...
      toast.error("Security protocols blocked registry access.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        ...
        toast.success("New supplier initialized in registry.");
      }
      resetForm();
      fetchSuppliers();
    } catch (error) {
      toast.error(error.response?.data?.message || "Protocol failure during commit.");
    }
  };
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Permanently erase this entity from the secure registry?")) return;
    try {
      await axios.delete(`${import.meta.env.VITE_BACKEND_URL}/api/suppliers/${id}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` }
      });
      toast.success("Entity purged from records.");
      fetchSuppliers();
    } catch (error) {
      toast.error("Purge authorization denied.");
    }
  };
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-slate-100">
        <div>
          <div className="flex items-center gap-2.5 mb-3">
            ...
            <p className="text-primary font-black uppercase tracking-[0.4em] text-[9px]">Supply Chain Infrastructure</p>
          </div>
          <h1 className="heading-premium text-2xl md:text-5xl leading-tight">
            Supplier <span className="italic font-medium text-slate-400">Portfolio</span>
          </h1>
          <p className="text-slate-400 font-medium mt-2 md:mt-3 text-sm md:text-base">Manage upstream logistic partners and preparation conduits.</p>
        </div>
        ...
        <div className="flex items-center gap-3.5">
          <div className="relative group">
            ...
            <input 
              ...
              className="pl-11 pr-5 py-3 md:py-3.5 bg-white border border-slate-200 rounded-xl md:rounded-2xl outline-none focus:ring-4 focus:ring-primary/5 focus:border-primary transition-all font-bold text-slate-900 w-full md:w-60 text-xs"
            />
          </div>
          <button 
            onClick={() => setIsFormOpen(!isFormOpen)}
            className="py-3.5 md:py-4 px-6 md:px-7 bg-slate-900 text-gold rounded-xl md:rounded-2xl font-black text-[10px] md:text-xs uppercase tracking-widest shadow-xl shadow-slate-200 hover:bg-primary hover:text-white transition-all flex items-center gap-2.5"
          >
            <Plus size={18} md:size={20} />
            <span className="hidden sm:inline">Initialize Partner</span>
          </button>
        </div>
      </header>
            ...
            <div className="flex items-center gap-4 mb-8 relative z-10">
              ...
              <div>
                <h2 className="text-xl md:text-2xl font-black text-slate-900 tracking-tight uppercase leading-none">
                  {editingId ? "Update Partner Specs" : "Partner Initialization"}
                </h2>
                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1 italic">Registry Protocol 7.4.x</p>
              </div>
            </div>

            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 md:gap-6 relative z-10">
              <div className="space-y-1.5 md:space-y-2">
                <label className="block text-[8px] md:text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Entity Name</label>
                <input 
              ...
              <div className="space-y-1.5 md:space-y-2">
                <label className="block text-[8px] md:text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Point Person</label>
                <input 
              ...
              <div className="space-y-1.5 md:space-y-2">
                <label className="block text-[8px] md:text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Comms Vector (Phone)</label>
                <input 
              ...
              <div className="space-y-1.5 md:space-y-2">
                <label className="block text-[8px] md:text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Email Node</label>
                <input 
              ...
              <div className="space-y-1.5 md:space-y-2 lg:col-span-2">
                <label className="block text-[8px] md:text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Logistic Hub (Address)</label>
                <input 
              ...
              <div className="space-y-1.5 md:space-y-2">
                <label className="block text-[8px] md:text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Material phylum</label>
              ...
              <div className="space-y-1.5 md:space-y-2">
                <label className="block text-[8px] md:text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Ops Status</label>
                <select 
              ...
              <div className="lg:col-span-4 flex justify-end gap-5 pt-4 border-t border-slate-50 mt-2">
                ...
                <button type="submit" className="py-4 md:py-4.5 px-10 md:px-12 bg-slate-900 text-white rounded-xl md:rounded-2xl font-black text-[10px] md:text-xs uppercase tracking-[0.3em] shadow-2xl shadow-slate-200 hover:bg-primary transition-all duration-500 hover:-translate-y-0.5">
                  {editingId ? "Synchronize Specs" : "Commit to Registry"}
                </button>
              </div>
            </form>
          </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/50">
                <th className="px-8 md:px-10 py-5 text-[9px] font-black text-slate-400 uppercase tracking-[0.3em]">Partner Entity</th>
                <th className="px-8 md:px-10 py-5 text-[9px] font-black text-slate-400 uppercase tracking-[0.3em]">Logistics Cat</th>
                <th className="px-8 md:px-10 py-5 text-[9px] font-black text-slate-400 uppercase tracking-[0.3em]">Comms Contact</th>
                <th className="px-8 md:px-10 py-5 text-[9px] font-black text-slate-400 uppercase tracking-[0.3em]">Ops Status</th>
                <th className="px-8 md:px-10 py-5 text-[9px] font-black text-slate-400 uppercase tracking-[0.3em] text-right">Registry Operations</th>
              </tr>
            </thead>
              ...
              {loading ? (
                <tr>
                  <td colSpan="5" className="px-12 py-32 text-center">
                    ...
                    <p className="font-black uppercase tracking-[0.4em] text-[10px] text-slate-400">Accessing Master Registry...</p>
                  </td>
                </tr>
                ...
                <tr>
                  <td colSpan="5" className="px-12 py-32 text-center text-slate-300">
                    ...
                    <p className="font-black uppercase tracking-widest text-xs tracking-[0.2em]">Registry is currently void.</p>
                  </td>
                </tr>
              ) : (
                    ...
                    <td className="px-8 md:px-10 py-5 md:py-6">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-[9px] font-bold text-slate-400 flex items-center gap-1.5 mt-1 px-1.5 py-0.5 bg-slate-50 rounded-md w-fit">
                            <MapPin size={10} className="text-primary"/> {s.address || "Hub undisclosed"}
                          </p>
                        </div>
                      </div>
                    </td>
                    <td className="px-8 md:px-10 py-5 md:py-6">
                      <span className="px-4 py-1.5 bg-slate-50 text-slate-600 border border-slate-100 rounded-full text-[10px] font-black uppercase tracking-widest leading-none">
                        {s.category}
                      </span>
                    </td>
                    <td className="px-8 md:px-10 py-5 md:py-6">
                      <div className="space-y-1.5">
                        <p className="text-xs font-black text-slate-900 flex items-center gap-2 uppercase tracking-tight">
                          <User size={12} className="text-slate-300" /> {s.contactPerson || "Anonymous"}
                        </p>
                        <div className="flex flex-col gap-0.5">
                          ...
                          {s.email && (
                            <p className="text-[9px] font-bold text-slate-400 flex items-center gap-1.5">
                              <ExternalLink size={10} className="text-primary" /> {s.email}
                            </p>
                          )}
                        </div>
                      </div>
