...
export default function AdminSupplierAccounts() {
  ...
  const handleSupplySubmit = async (e) => {
    ...
    try {
      ...
      await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/purchases`, {
        ...newSupply,
        sellingPrice: parseFloat(newSupply.unitPrice) || 0,
        discount: 0,
        supplier: id
      }, {
        headers: { Authorization: `Bearer ${token}` }
  ...
  const handleRecordUpdate = async (e) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.put(`${import.meta.env.VITE_BACKEND_URL}/api/purchases/${recordFormData._id}`, {
        ...recordFormData,
        sellingPrice: parseFloat(recordFormData.unitPrice) || 0,
        discount: 0
      }, {
        headers: { Authorization: `Bearer ${token}` }
      });
          ...
          <div className="flex items-center gap-3">
            <div className="flex flex-col">
              ...
              <p className="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-widest">
                {log.quantity} units @ Cost: Rs. {formatCurrency(log.unitPrice)}
              </p>
            </div>
          </div>
        </td>
